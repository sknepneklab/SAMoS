...
class Circle:
  def __init__(self, R, N, v, poly,boundary,bdens,a0):
    ...
    self.Nbound = int(bdens*2.0*pi*R/2.0) # Dense boundary wall, linear packing fraction 2.0 (for stiff glued boundaries)
    area=a0*numpy.random.uniform(1-0.5*self.poly,1+0.5*self.poly,self.N+self.Nbound)
    self.particles = [Particle(i,area[i],1) for i in range(N)]
    self.boundparticles = [Particle(i,area[i],2) for i in range(N,N+self.Nbound)]
...
parser.add_argument("-a", "--area",  type=float, default=pi, help="cell area")

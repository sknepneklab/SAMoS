...
class Plane:
  ...
  def __init__(self, Lx, Ly, N, poly, ratio, v,radius):
    self.L = (Lx,Ly)
    self.N = N
    self.ratio=ratio
  ...
  def set_radius(self,poly,radius):
    for i in range(self.N):
      self.particles[i].R = radius*uniform(1-0.5*poly,1+0.5*poly)
...
parser.add_argument("-i", "--radius", type=float, default=1.0, help="mean particle radius")
...
N = int(round(1.0/(pi*args.radius**2)*args.lx*args.ly*args.phi))
...
p = Plane(args.lx, args.ly, N, args.poly, args.ratio, args.vavr,args.radius)
p.write(args.output)

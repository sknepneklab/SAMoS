...
for phi in phival:
  for v0 in v0val:
    for nu in nuval:
      for k in kval:
        confdir = filebase + "nodiv_vertex/p0_" + phi + \
            "/nu_" + nu + "/v0_" + v0 + "/k_" + k + "/conf2/"
        radiusfile = "vertex_input.dat"
        prefix = "cell_00"
        conffile = 'vertex.conf'
        sim = SimRun(confdir, conffile, prefix, radiusfile, 50, False, True, True, usetype=1)
        ...
        outpickle = fileout + "Velcorr_" + "phi" + phi + "_nu" + \
            nu + "_v0" + v0 + "_k" + k + "vertex_expsim.p"
        print outpickle
        data = {"nu": nu, 
                "v0": v0, 
                "k": k, 

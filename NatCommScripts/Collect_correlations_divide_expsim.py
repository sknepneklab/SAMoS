...
for phi in phival:
	for v0 in v0val:
		for nu in nuval:
      for k in kval:
        confdir = filebase + "divide/rdeath_" + rdeath + "/nu_" + nu +"/v0_" + v0 + "/ratio_" + mult + "/k_" + k + "/conf3/"
        radiusfile = "plane.txt"
        prefix = "cornea_cells_00"
        conffile = 'epithelial_divide_tracers_expsim.conf'
        sim = SimRun(confdir, conffile, prefix, radiusfile, 50, True, False, True)
        ...
        for l in range(0,sim.Nsnap,10):
          if l % 10 == 0:
            print l
          # qrad,valrad,Sqrad=FourierTransVel(self,whichframe,qmax=0.3,verbose=True):
        ...
        if plotting:
          ...
          plt.xlabel('q')
          plt.ylabel('$<|v(q)|^2>')
          plt.title('Computational Fourier velocity, v_0 = ' + v0 )
                    
        tval_auto, velauto, v2av = sim.getVelAuto(False)
        tval_msd, msd = sim.getMSD(False)
        qval = np.array([0.3,0.3,0.0])/np.sqrt(2)

...
void Dump::dump_input()
{
  ...
  {
    Particle& p = m_system->get_particle(particles[i]);
    m_out << format("%5d  %3d  %2d  %8.4f  %10.6f  %10.6f  %10.6f  %10.6f  %10.6f  %10.6f  %10.6f  %10.6f  %10.6f  %10.6f  %3d  %3d  %3d") % p.get_id() % p.molecule % p.get_type() % p.get_radius() % p.x % p.y % p.z % p.vx % p.vy % p.vz % p.nx % p.ny % p.nz % p.get_length() % p.ix % p.iy % p.iz << endl;
  }
}

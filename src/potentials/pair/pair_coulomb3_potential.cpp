...
/*!
 * \file pair_coulomb3_potential.cpp
 * \author Rastko Sknepnek, sknepnek@gmail.com
 * \date 22-Jun-2022
 * \brief Implementation of PairCoulomb3Potential class
 */

#include "pair_coulomb3_potential.hpp"

void PairCoulomb3Potential::compute(double dt)
{
    int N = m_system->size();
    ...
    if (m_system->record_force_type())
        this->reset_force_types("coulomb3");
...
//#endif
                double r = sqrt(r_sq);
                // Handle potential
                double potential_energy = alpha / (r_sq*r);
                m_potential_energy += potential_energy;
                // Handle force
                double r_5 = r * r_sq * r_sq;
                double force_factor = 3.0 * alpha / r_5;
// #ifdef __AVX2__
...
#pragma omp critical
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        pi.fz += fz[i];
        if (m_system->record_force_type())
          pi.add_force_type("coulomb3",fx[i],fy[i],fz[i]);
    }
}

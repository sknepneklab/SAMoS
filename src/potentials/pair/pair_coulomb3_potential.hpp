 ...
 * \date 22-Jun-2022
 * \brief Declaration of PairCoulomb3Potential class
 */ 

#ifndef __PAIR_COULOMB3_POTENTIAL_HPP__
#define __PAIR_COULOMB3_POTENTIAL_HPP__
...
struct Coulomb3Parameters
{
  double alpha;
};

/*! PairCoulomb3Potential implements Coulomb-like potential, but with the 1/r^3 decay. Note that there is no hard-core term
 *  and the simulation will fail for system with particles with opposite charges. The potential is given as
 *  \f$ U_{Coul}\left(r_{ij}\right) = \frac{\alpha}{r_{ij}^3} \f$,
 *  where \f$ \alpha \f$ is the potential strength,  and \f$ r_{ij} \f$ is the 
 *  inter-particle distance.
 */
class PairCoulomb3Potential : public PairPotential
{
public:
  ...
  PairCoulomb3Potential(SystemPtr sys, MessengerPtr msg, NeighbourListPtr nlist, ValuePtr val, pairs_type& param) : PairPotential(sys, msg, nlist, val, param)
  {
    ...
    m_msg->write_config("potential.pair.Coulomb3.alpha",lexical_cast<string>(m_alpha));
    
    m_pair_params = new Coulomb3Parameters*[m_ntypes];
    for (int i = 0; i < m_ntypes; i++)
    {
      m_pair_params[i] = new Coulomb3Parameters[m_ntypes];
  ...
  void set_pair_parameters(pairs_type& pair_param)
  {
    ...
    m_msg->write_config("potential.pair.Coulomb3.type_"+pair_param["type_1"]+"_and_type_"+pair_param["type_2"]+".alpha",lexical_cast<string>(param["alpha"]));
...
private:
  ...
  Coulomb3Parameters** m_pair_params;   //!< type specific pair parameters 
    
};

typedef shared_ptr<PairCoulomb3Potential> PairCoulomb3PotentialPtr;

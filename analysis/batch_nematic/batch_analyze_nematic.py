...
files = sorted(glob(basefolder+'sphere_J*.dat'))[skip:]
defects=np.zeros((len(files),12))
ndefect=np.zeros((len(files),1))
u=0
for f in files:
	outname =outfolder + 'frame_data' + str(u)+'.vtk'
	...
	outname =outfolder + 'frame_defects' + str(u)+'.vtk'
	print outname
	writeDefects(defects0,ndefect0,outname)
	u+=1
	
outfile2=outfolder + 'defects.dat'

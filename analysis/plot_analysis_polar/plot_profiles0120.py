...
for j in range(len(JList)):
	...
	for i in range(len(vList)):
		...
		ispretty=isdata[0:len(isdata):5]
		# Corrected
		...
		if j==0:
			if usecolumn==2:
				...
				plt.plot(profiles[0,ispretty],profiles[usecolumn,ispretty]/float(vList[i]),color=testmap(i), linestyle='none',marker='o')
				plt.text(0.75,1.0,r'$J=10$')
			else:
				plt.plot(profiles[0,isdata],profiles[usecolumn,isdata],color=testmap(i), linestyle='solid',label=vList[i])
				plt.plot(profiles[0,ispretty],profiles[usecolumn,ispretty],color=testmap(i), linestyle='none',marker='o')
		else:
			if usecolumn==2:
				if j==1:
					plt.plot(profiles[0,isdata],profiles[usecolumn,isdata]/float(vList[i]),color=testmap(i), linestyle='solid')
					plt.plot(profiles[0,ispretty],profiles[usecolumn,ispretty]/float(vList[i]),color=testmap(i), linestyle='none',marker='*')
					plt.text(0,1.07,r'$J=1$')
				else:
					plt.plot(profiles[0,isdata],profiles[usecolumn,isdata]/float(vList[i]),color=testmap(i), linestyle='solid')
					plt.plot(profiles[0,ispretty],profiles[usecolumn,ispretty]/float(vList[i]),color=testmap(i), linestyle='none',marker='v')
					plt.text(-0.4,0.85,r'$J=0.1$')
			else:
				plt.plot(profiles[0,isdata],profiles[usecolumn,isdata],color=testmap(i), linestyle='solid')
				if j==1:
					plt.plot(profiles[0,ispretty],profiles[usecolumn,ispretty],color=testmap(i), linestyle='none',marker='*')
				else:
					plt.plot(profiles[0,ispretty],profiles[usecolumn,ispretty],color=testmap(i), linestyle='none',marker='v')
	#if usecolumn<=8:
		#plt.ylim(0,1.25*profiles[usecolumn,nbin/2])
	if usecolumn==9:
		if j==0:
			plt.plot(2*profiles[0,isdata],0.1*2*profiles[0,isdata],'k--')
			plt.text(0.5,-0.05,r'$J=10, s=0.1$')
			#plt.ylim(-0.4,0.4)
		if j==1:
			plt.plot(2*profiles[0,isdata],0.45*2*profiles[0,isdata],'k--')
			plt.text(0.5,0.4,r'$J=1, s=0.45$')
		if j==2:
			plt.plot(2*profiles[0,isdata],1.25*2*profiles[0,isdata],'k--')
			plt.text(-0.25,0.8,r'$J=0.1, s=1.25$')

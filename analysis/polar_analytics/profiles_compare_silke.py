...
for j in range(len(JList)):
	...
	for i in range(len(vList)):
		...
		if usecolumn==2:
			plt.plot(profiles[0,isdata],profiles[usecolumn,isdata]/float(vList[i]),color=testmap(j), linestyle='solid',label=vList[i])
		else:
			plt.plot(profiles[0,isdata],profiles[usecolumn,isdata],color=testmap(j), linestyle='solid',label=vList[i])
	#if usecolumn<=8:
...
for i in range(len(vList)):	
	...
	isdata=[index for index,value in enumerate(profiles[1,:]) if (value >0)]
	if usecolumn==2:
		plt.plot(profiles[0,isdata],profiles[usecolumn,isdata]/float(vList[i]),color=testmap(2), linestyle='solid',label=JList[j])
	else:
		plt.plot(profiles[0,isdata],profiles[usecolumn,isdata],color=testmap(2), linestyle='solid',label=JList[j])
if usecolumn==9:

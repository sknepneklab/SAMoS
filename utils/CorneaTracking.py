...
class TrackDefects:
    ...
    def __init__(self,filename,R,skip):
	data=pickle.load(open(filename))
	ndefects0=data["numdefects_n"]
	defects0=data["defects_n"]
	self.ndefects=ndefects0[skip:]
	self.defects=defects0[skip:]
	par=param(R)
	...
	for u in range(0,len(self.defects1)):
	    ...
	    ismone=[index for index,value in enumerate(charge) if value==-1]
    ...
    def plotDefects(self,title,coneangle,merged,polar,verbose=False):
	...
	for u in range(0,nsnaps):
	    ...
	    # Sort them into defects of charge +1/2, -1/2 and 1.0
	    ismone=[index for index,value in enumerate(charge) if value==-1.0]
	    isone=[index for index,value in enumerate(charge) if value==1.0]    
	    # Get the angles associated to the defects
	    angles[:,0],angles[:,1],etheta,ephi=self.geom.TangentBundle(rval)  
	    if polar:
		ax.plot(angles[ismone,1],angles[ismone,0], color='r',linestyle='none',marker='.')
		ax.plot(angles[isone,1],angles[isone,0], color='k',linestyle='none',marker='.')
	    else:
		# first angle, then second angle
		pu=[u for k in ismone]
		ax1.plot(pu,angles[ismone,0], color='r',linestyle='none',marker='.')
		ax2.plot(pu,angles[ismone,1], color='r',linestyle='none',marker='.')
		pu=[u for k in isone]
		ax1.plot(pu,angles[isone,0], color='k',linestyle='none',marker='.')
		ax2.plot(pu,angles[isone,1], color='k',linestyle='none',marker='.')

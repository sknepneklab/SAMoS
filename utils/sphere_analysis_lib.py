...
class Configuration:
    ...
    def compute_energy_and_pressure_rastko(self,r):
        eng = np.zeros(len(r))
    ...
    def compute_energy_and_pressure(self):
        eng = np.zeros(len(self.rval))
        press = np.zeros(len(self.rval))
        stress = np.zeros((len(self.rval),3,3))
        #dist = sd.cdist(r,r)
        dmax=4*self.param.sigma**2
        for i in range(len(r)):
            #for i in range(10):
            ...
            Fvec=self.param.k*((diff/dr).transpose()*(drvec).transpose()).transpose()
            for u in range(3):
                for v in range(3):

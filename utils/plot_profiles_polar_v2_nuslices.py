...
for i in range(len(vList)):
    ...
    plt.errorbar(nuval,orderpar,yerr=dorder,color=testmap(i), linestyle='solid',marker='s',markersize=10,label=vList[i])
...
for i in range(len(vList)):
    ...
    plt.plot(nuList,mdefects[:,1],color=testmap(i),linestyle='solid',label=vList[i])
    plt.plot(nuList,mdefects[:,0],color=testmap(i),linestyle='--')
plt.xlabel(r'$\nu$')
plt.ylabel('defects')
plt.title('Defects')
plt.legend()
